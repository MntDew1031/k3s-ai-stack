 ...
 **ThinkCentre micro computers. Each is powered by a low voltage**
...
The cluster was built to host a privacy-focused AI inference pipeline and custom integration tooling.
  ...
  * **Frontend:** Open WebUI deployed with strict role based access control (RBAC) to separate administrative and guest model access.
...
Changes pushed to the `main` branch of this repository are automatically reconciled by FluxCD. The cluster architecture heavily utilizes Kustomize to patch base manifests and manage namespace configurations without duplicating code.
...
1. **Basic Use of the AI models**
   ...
   * Select what model you want to use by toggling the model drop down on the top left of the new chat panel.
     * (Pro tip, I reccomend loading up qwen2.5 for the first test so the second is quicker)
...
2. **Testing the MCP Tool**
   ...
   * This one will most likey take a bit longer as this is running on decade old hardware and most likely thermal throttling :)
